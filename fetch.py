...
Установка: pip install requests pycountry reverse_geocoder
...
try:
    ...
    HAS_RG = False
    print("⚠️  reverse_geocoder не установлен — таблица путаницы недоступна")
    print("   pip install reverse_geocoder")


# ── Конфиг ───────────────────────────────────────────────────────────────────

NCFA_COOKIE   = os.environ.get("GEOGUESSR_NCFA", "")
MAX_DUELS     = 9999
REQUEST_DELAY = 0.3
...
def parse_duel(duel: dict, my_id: str) -> list[dict] | None:
    ...
    for i, rnd in enumerate(rounds_data):
        ...
        results.append({
            ...
            "guessed_cc":     "",   # заполнится в batch_geocode
            "guessed_country": "",
            "score":          score,
...
def batch_geocode(all_rounds: list[dict]) -> None:
    """Один вызов rg.search() на все раунды без guessed_cc."""
    if not HAS_RG:
        return
    to_fill = [r for r in all_rounds if not r.get("guessed_cc") and r.get("guess_lat") is not None]
    if not to_fill:
        return
    print(f"🌍 Геокодирую {len(to_fill)} гессов одним батчем...")
    coords = [(r["guess_lat"], r["guess_lng"]) for r in to_fill]
    try:
        results = rg.search(coords, verbose=False)
        for r, res in zip(to_fill, results):
            cc = res.get("cc", "").upper()
            r["guessed_cc"]      = cc
            r["guessed_country"] = cc_to_name(cc)
    except Exception as e:
...
def compute_stats(rounds: list[dict]) -> tuple[dict, list[tuple]]:
    ...
    # confusion: (actual, guessed) → count
    ...
    for r in rounds:
        ...
        gc = r.get("guessed_country", "")
        if gc and gc != "Unknown" and gc != c:
            confusion[(c, gc)] += 1
    ...
    for country, s in raw.items():
        ...
        if HAS_RG:
            country_correct = sum(
                1 for r2 in rounds
                if r2["actual_country"] == country
                and r2.get("guessed_cc")
                and r2["guessed_cc"] == r2["actual_cc"]
            )
        else:
            country_correct = sum(1 for sc in scores if sc >= 4500)
        country_pct = round(country_correct / n * 100, 1)
        result[country] = {
            "rounds":      n,
            "won":         s["won"],
    ...
    # топ путаниц отсортированные по частоте
    top_confusion = sorted(confusion.items(), key=lambda x: -x[1])

    return result, top_confusion
...
def make_tab_html(stats: dict, rounds: list[dict], top_confusion: list) -> str:
    if not rounds:
    ...
    <table data-sortable>
      <thead><tr>
        ...
        <th class="num" title="% раундов где угадал страну правильно">Country%</th>
    ...
    # Таблица путаницы
    if top_confusion and HAS_RG:
        conf_rows = "".join(
            f'<tr><td class="bold">{a}</td>'
            f'<td style="color:#475569;padding:0 .5rem">→</td>'
            f'<td class="bold">{b}</td>'
            f'<td class="num" style="color:#fb923c;font-weight:700">{n}×</td></tr>'
            for (a, b), n in top_confusion[:25]
        )
        confusion_table = f"""
        <h2 style="margin-top:2rem">🔀 Most Confused Countries</h2>
        <p style="color:#475569;font-size:.8rem;margin-bottom:.75rem">
          Когда была страна A — я гессил B
        </p>
        <table style="max-width:420px">
          <thead><tr>
            <th>Actual</th><th></th><th>Guessed as</th><th class="num">Times</th>
          </tr></thead>
          <tbody>{conf_rows}</tbody>
        </table>"""
    elif not HAS_RG:
        confusion_table = '<p style="color:#475569;font-size:.85rem;margin-top:2rem">Установи <code>reverse_geocoder</code> для таблицы путаницы.</p>'
    else:
        confusion_table = ""

    return summary + table + confusion_table

def generate_html(move_rounds, nomove_rounds, username) -> str:
    move_stats,   move_conf   = compute_stats(move_rounds)   if move_rounds   else ({}, [])
    nomove_stats, nomove_conf = compute_stats(nomove_rounds) if nomove_rounds else ({}, [])
    move_html   = make_tab_html(move_stats,   move_rounds,   move_conf)
    nomove_html = make_tab_html(nomove_stats, nomove_rounds, nomove_conf)
...
<div class="legend">
  Country% — доля раундов где угадана правильная страна.<br>
...
def main():
    global NCFA_COOKIE
    if not NCFA_COOKIE:
        # Локальный запуск — спрашиваем интерактивно
        if sys.stdin.isatty():
            print("DevTools → Application → Cookies → geoguessr.com → _ncfa")
            NCFA_COOKIE = input("_ncfa: ").strip()
        else:
            sys.exit("❌ Установи переменную окружения GEOGUESSR_NCFA")
    ...
    for i, gid in enumerate(duel_ids):
        if gid in cache and cache[gid] is not None:
            rounds = cache[gid]
        else:
            duel   = get_duel(gid, session)
            rounds = parse_duel(duel, my_id) if duel else None
            cache[gid] = rounds  # кэшируем только если есть данные
            if rounds is None:
                del cache[gid]
            new_fetched += 1
            if new_fetched % 25 == 0:
                save_cache(cache)
                print(f"  [{i+1}/{len(duel_ids)}] {len(all_rounds)} раундов, кэш сохранён...")
            elif new_fetched % 5 == 0:
                print(f"  [{i+1}/{len(duel_ids)}]...", end="\r")
            time.sleep(REQUEST_DELAY)
    ...
    # Батч-геокодирование всех гессов за один вызов
    batch_geocode(all_rounds)

    # Сохраняем геокодированные данные обратно в кэш
    geocoded_by_game: dict[str, list] = defaultdict(list)
    for r in all_rounds:
        geocoded_by_game[r["game_id"]].append(r)
    for gid, rounds in geocoded_by_game.items():
        cache[gid] = rounds
    save_cache(cache)
